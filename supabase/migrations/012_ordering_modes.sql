-- ============================================================
-- 012_ordering_modes.sql
--
-- Adds takeaway / delivery as first-class order channels.
--
-- Changes:
--   1. business_settings — two new boolean columns:
--        takeaway_enabled (default true)
--        delivery_enabled (default false)
--   2. get_customer_menu — expose the two new columns so the
--      customer menu page knows which channels are open.
--   3. atomic_checkout — generalise the table-id resolution so
--      ANY non-UUID string (walk-in, takeaway, delivery, …)
--      passes through safely, instead of only special-casing
--      'walk-in' with a NULLIF cast that blows up on other strings.
-- ============================================================
...
-- 3. atomic_checkout — safe non-UUID table_id resolution ────────────────────
--
-- Previous version used NULLIF(p_table_id, 'walk-in')::uuid which only
-- special-cased 'walk-in'. Passing 'takeaway' or 'delivery' would attempt
-- to cast the literal string to UUID and raise an invalid_text_representation
-- error. Replace with a regex guard so any non-UUID string passes through.
...
BEGIN
  ...
  -- Determine whether p_table_id is a real UUID (dine-in table) or a keyword
  -- (walk-in, takeaway, delivery). Cast is only safe when it looks like a UUID.
  ...
  IF v_table_name IS NULL OR NOT v_is_table_uuid THEN
    -- Keyword modes: produce a clean display name
  ...
  -- Only mark a real table as occupied (not walk-in / takeaway / delivery)
  IF v_is_table_uuid THEN
    UPDATE tables
       SET status = 'occupied'
